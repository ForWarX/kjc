...
if (!$smarty->is_cached('goods.dwt', $cache_id) || true) // 记得去掉true！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！
{
	...
	$smarty->assign('same-cat-product', get_same_cat($goods_id, $goods['cat_id'], 3));
...
function get_same_cat($goods_id, $cat_id, $quantity){
	...
	foreach ($GLOBALS['db']->getALL($sql) as $single){
		...
		$handle_array[] = array('goods_thumb' => $single['goods_thumb'],'goods_name' => $single['goods_name'],'price' => price_format($price),'url' => 'goods.php?id='.$single['goods_id']);	
	}
