...
<head>
    ...
    <link href="themes/ningbo/style.css" rel="stylesheet" type="text/css" />
...
<body onload="formCart.reset();"><?php echo $this->fetch('library/page_header.lbi'); ?><?php echo $this->fetch('library/ur_here.lbi'); ?>
<div class="blank"></div>
<div class="block">
    ...
    <div class="flowBox">
        ...
        <form id="formCart" name="formCart" method="post" action="flow.php">
            <table width="99%" align="center" border="0" cellpadding="5" cellspacing="1" bgcolor="#dddddd">
                <tr>
                    <th bgcolor="#ffffff"><input type="checkbox" id="checkAll" <?php if ($this->_var['is_all_checked'] == 1): ?> checked <?php endif; ?>/> 全选</th>
...
?>
                <tr>
                    ...
                    <td bgcolor="#ffffff" align="center">
                        <input type="checkbox" name="goodsSelect[<?php echo $this->_var['goods']['rec_id']; ?>]" value="<?php echo $this->_var['goods']['goods_id']; ?>"  <?php if ($this->_var['goods']['is_selected'] == 1): ?>checked<?php endif; ?> />
                    </td>
                    <td bgcolor="#ffffff" align="center">
                        ...
                        <?php else: ?>
                        <div style="position:relative;width: 80px;"><a href="goods.php?id=<?php echo $this->_var['goods']['goods_id']; ?>" target="_blank"><img style="width:80px; height:80px;" src="<?php echo $this->_var['goods']['goods_thumb']; ?>" border="0" title="<?php echo htmlspecialchars($this->_var['goods']['goods_name']); ?>" /></a><?php if ($this->_var['goods']['kj_sn']): ?><img src="themes/ningbo/images/kj_logo/kj30.png" class="kj-img-small"/><?php endif; ?></div><br />
                        <a href="goods.php?id=<?php echo $this->_var['goods']['goods_id']; ?>" target="_blank"><?php echo $this->_var['goods']['goods_name']; ?></a>
                        <?php endif; ?>
                        <?php if ($this->_var['goods']['parent_id'] > 0): ?>
                        <span style="color:#EF5A2C">（<?php echo $this->_var['lang']['accessories']; ?>）</span>
                        <?php endif; ?>
                        <?php if ($this->_var['goods']['is_gift'] > 0): ?>
                        <span style="color:#EF5A2C">（<?php echo $this->_var['lang']['largess']; ?>）</span>
                        <?php endif; ?>
                        <?php elseif ($this->_var['goods']['goods_id'] > 0 && $this->_var['goods']['extension_code'] == 'package_buy'): ?>
                        <a href="javascript:void(0)" onclick="setSuitShow(<?php echo $this->_var['goods']['goods_id']; ?>)"><?php echo $this->_var['goods']['goods_name']; ?><span style="color:#EF5A2C;">（<?php echo $this->_var['lang']['remark_package']; ?>）</span></a>
...
?>
                    ...
                    <td align="right" bgcolor="#ffffff"><span name="subtotal"><?php echo $this->_var['goods']['subtotal']; ?></span></td>
                ...
                <?php if (count ( $this->_var['kj_goods_list'] ) > 0): ?>
                <tr>
                    <th colspan="2" align="left">跨境购商品列表</th>
                </tr>
                <?php $_from = $this->_var['kj_goods_list']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('', 'goods');if (count($_from)):
    foreach ($_from AS $this->_var['goods']):
?>
                <tr>
                    <td bgcolor="#ffffff" align="center">
...
?>
                ...
                <tr>
                    <td bgcolor="#ffffff" align="center">
                        <input type="checkbox" name="kj_agree" id="kj_agree"/>
                    </td>
                    <td colspan="6" align="center">我已认真阅读并同意<a href="article.php?id=36" target="_blank">《跨境电子商务购物协议》</a></td>
                </tr>
            ...
            <table width="99%" align="center" border="0" cellpadding="5" cellspacing="1" bgcolor="#dddddd">
                <tr>
                    <td bgcolor="#ffffff">
                        ...
                        <span id="shopping_money"><?php echo $this->_var['shopping_money']; ?></span>
            ...
            <input type="hidden" name="update_selected" value="no" />
        </form>
        <script>
            function updateCart(){
                var f=document.forms["formCart"];
                f.step.value="update_cart";
            }
            function checkout(){//跨境城购物车改造


                var f=document.forms["formCart"];
                /*购物车改造2 398行开始共计8行*/
                if (f.kj_agree){
                    if (f.kj_agree.checked == false){
                        alert("请您同意协议后再进行购物，谢谢合作！");
                        return;
                    }
                }
                /*购物车改造2 结束*/
                f.step.value="checkout";
                f.update_selected.value="yes";
                $("#formCart").find("input[name='submit']").click();
                f.step.value="update_cart";
                f.update_selected.value="no";
            }
            //购物车改造 全选框操作，选择框勾选金额的变动
            $(function(){
                var currency_format="<?php echo $this->_var['currencyFormat']; ?>";
                var shopping_money_lang="<?php echo $this->_var['shopping_money_lang']; ?>";
                $("#checkAll").click(function(){
                    if($("#checkAll").prop("checked")){
                        $("input[name^=goodsSelect]").prop("checked",true);
                    }else{
                        $("input[name^=goodsSelect]").prop("checked",false);
                    }
                    computeShoppingMoney();
                });
                $("input[name^=goodsSelect]").bind("click",function(){
                    computeShoppingMoney();
                });
                //计算选择商品的购物金额
                function computeShoppingMoney(){
                    var total=0.00;
                    $("span[name=subtotal]").each(function(idx,e){
                        var subtotal=$(e).html().split(currency_format)[1];
                        if( $("input[name^=goodsSelect]").eq(idx).prop("checked"))
                            total+=parseFloat(subtotal);
                    });
                    $("#shopping_money").html(shopping_money_lang+fmoney(total,2));
                }
                function fmoney(s, n){//格式化金额
                    n = n > 0 && n <= 20 ? n : 2;
                    ...
                    for(i = 0; i < l.length; i ++ ){
                        //t += l[i] + ((i + 1) % 3 == 0 && (i + 1) != l.length ? "," : ""); 三位分割
                        t += l[i] + ((i + 1) % 3 == 0 && (i + 1) != l.length ? "" : "");
                    }
                    return currency_format+t.split("").reverse().join("") + "." + r;
                }
        ...
        <table width="99%" align="center" border="0" cellpadding="5" cellspacing="0" bgcolor="#dddddd">
            <tr>
                <td bgcolor="#ffffff"><a href="./"><img src="themes/ningbo/images/continue.gif" alt="continue" /></a></td>
                <!-- <td bgcolor="#ffffff" align="right"><a href="flow.php?step=checkout"><img src="themes/ningbo/images/checkout.gif" alt="checkout" /></a></td> 跨境城购物车改造 -->
                <td bgcolor="#ffffff" align="right"><a href="javascript:checkout();"><img src="themes/ningbo/images/checkout.gif" alt="checkout" /></a></td>
            </tr>
        </table>
...
?>
            <tr>
                ...
                <td bgcolor="#ffffff" align="center" width="100"><a href="javascript:addToCart(<?php echo $this->_var['goods']['goods_id']; ?>)"><?php echo $this->_var['lang']['collect_to_flow']; ?></a></td>
            </tr>
            <?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>
        </table>
    </div>
...
?>
                    <ul class="clearfix">
                        ...
                        <li>
                            ...
                            <a href="javascript:fittings_to_flow(<?php echo $this->_var['fittings']['goods_id']; ?>,<?php echo $this->_var['fittings']['parent_id']; ?>)"><img src="themes/ningbo/images/bnt_buy.gif" alt="<?php echo $this->_var['lang']['collect_to_flow']; ?>" /></a>
                        </li>
                    </ul>
                    <?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>
                </div>
...
?><br />
                    ...
                    <tr>
                        ...
                        <td align="center" bgcolor="#ffffff"><input type="image" src="themes/ningbo/images/bnt_cat.gif" alt="Add to cart"  border="0" /></td>
                    </tr>
                    <?php endif; ?>
                </table>
...
?>
    ...
    <form action="flow.php" method="post" name="theForm" id="theForm" onsubmit="return checkOrderForm(this)">
        ...
        <div class="flowBox">
            <h6><span><?php echo $this->_var['lang']['goods_list']; ?></span><?php if ($this->_var['allow_edit_cart']): ?><a href="flow.php"><?php echo $this->_var['lang']['modify']; ?></a><?php endif; ?></h6>
            <table width="99%" align="center" border="0" cellpadding="5" cellspacing="1" bgcolor="#dddddd">
                <tr>
                    <th bgcolor="#ffffff"><?php echo $this->_var['lang']['goods_name']; ?></th>
                    <th bgcolor="#ffffff"><?php echo $this->_var['lang']['goods_attr']; ?></th>
...
?>
                        ...
                        <?php endif; ?>
                        <?php endif; ?>
                        <?php if ($this->_var['goods']['is_shipping']): ?>(<span style="color:#EF5A2C"><?php echo $this->_var['lang']['free_goods']; ?></span>)<?php endif; ?>
                    </td>
                ...
                <tr>
                    <td bgcolor="#ffffff" colspan="7">
                        ...
                        &nbsp;总重量：<?php echo $this->_var['cart_weight_price']['formated_weight']; ?>
                    </td>
                </tr>
                <?php endif; ?>
            </table>
        ...
        <div class="blank"></div>
        <div class="flowBox">
            <h6><span><?php echo $this->_var['lang']['consignee_info']; ?></span><a href="flow.php?step=consignee"><?php echo $this->_var['lang']['modify']; ?></a></h6>
            <table width="99%" align="center" border="0" cellpadding="5" cellspacing="1" bgcolor="#dddddd">
                <tr>
        ...
        <?php if ($this->_var['real_authenty'] == 1): ?>
        <div class="flowBox">
            <h6><span>跨境订单必须进行实名信息认证</span></h6>
            <table width="99%" align="center" border="0" cellpadding="5" cellspacing="1" bgcolor="#dddddd" id="shippingTable">
                <tr>
                    <th bgcolor="#ffffff" width="10%"><span style="color:#EF5A2C;">*</span>姓名</th>
                    <th bgcolor="#ffffff" width="12%"><input type="text" name="real_name" maxlength="32" value="<?php echo $this->_var['real_info']['real_name']; ?>"/></th>
                    <th bgcolor="#ffffff" width="10%"><span style="color:#EF5A2C;">*</span>身份证号</th>
                    <th bgcolor="#ffffff" width="20%"><input type="text" name="id_num" size="30" maxlength="18" value="<?php echo $this->_var['real_info']['real_id']; ?>"/></th>
                    <th bgcolor="#ffffff" width="10%"><span style="color:#EF5A2C;">*</span>手机号</th>
                    <th bgcolor="#ffffff" width="13%"><input type="text" name="phone" size="15" maxlength="11" value="<?php echo $this->_var['real_info']['real_phone']; ?>"/></th>
                    <th bgcolor="#ffffff" width="10%">邮箱</th>
                    <th bgcolor="#ffffff" width="15%"><input type="text" name="identy_email" size="25" maxlength="32" value="<?php echo $this->_var['real_info']['real_email']; ?>"/></th>
                </tr>
            </table>
        </div>
        <div class="blank"></div>
...
?>
                <tr>
                    <td bgcolor="#ffffff" valign="top"><input name="shipping" type="radio" value="<?php echo $this->_var['shipping']['shipping_id']; ?>" <?php if ($this->_var['order']['shipping_id'] == $this->_var['shipping']['shipping_id']): ?>checked="true"<?php endif; ?> supportCod="<?php echo $this->_var['shipping']['support_cod']; ?>" insure="<?php echo $this->_var['shipping']['insure']; ?>" kjAmount="<?php echo $this->_var['kj_goods_amount']; ?>" kjTax="<?php echo $this->_var['kj_goods_tax']; ?>" onclick="selectShipping(this)" />
                    </td>
...
?>
                ...
                <tr>
                    <td valign="top" bgcolor="#ffffff"><input type="radio" name="payment" value="<?php echo $this->_var['payment']['pay_id']; ?>" <?php if ($this->_var['order']['pay_id'] == $this->_var['payment']['pay_id']): ?>checked<?php endif; ?> isCod="<?php echo $this->_var['payment']['is_cod']; ?>" kjAmount="<?php echo $this->_var['kj_goods_amount']; ?>" kjTax="<?php echo $this->_var['kj_goods_tax']; ?>" onclick="selectPayment(this)" <?php if ($this->_var['cod_disabled'] && $this->_var['payment']['is_cod'] == "1"): ?>disabled="true"<?php endif; ?>/></td>
                    <td valign="top" bgcolor="#ffffff"><img src="/images/pay_<?php echo $this->_var['payment']['pay_code']; ?>.gif"/></td>
                ...
                <tr>
                    ...
                    <td bgcolor="#ffffff" align="center" style="font-size:20px;color: #58595B;"><strong>P币支付</strong></td>
                    <td bgcolor="#ffffff"><input name="integral" type="text" class="input" id="ECS_INTEGRAL" onblur="changeIntegral(this.value)" value="<?php echo empty($this->_var['order']['integral']) ? '0' : $this->_var['order']['integral']; ?>" size="10" />
                        您有 <?php echo empty($this->_var['your_integral']) ? '0' : $this->_var['your_integral']; ?> <?php echo $this->_var['points_name']; ?>，本订单需要 <?php echo $this->_var['order_max_integral']; ?> <?php echo $this->_var['points_name']; ?>。<?php if ($this->_var['order_max_integral'] > $this->_var['your_integral']): ?><a href="user.php?act=pcoin_deposit"<?php if ($this->_var['action'] == 'pcoin_deposit'): ?>class="curs"<?php endif; ?>><img src="themes/ningbo/images/pcoin.gif" width="12px">充值</a><?php endif; ?><br/><span id="ECS_INTEGRAL_NOTICE" class="notice" style="font-weight:bold;"></span></td>
                    <td bgcolor="#ffffff"></td>
                </tr>
                <?php endif; ?>
            </table>
...
?>
        ...
        <div class="flowBox">
            ...
            <table width="99%" align="center" border="0" cellpadding="5" cellspacing="1" bgcolor="#dddddd">
                ...
                <tr>
                    <td width="20%" bgcolor="#ffffff"><strong>使用余额（<a href="user.php?act=account_deposit" style="color:#EF5A2C;">充值</a>）: </strong></td>
                    <td bgcolor="#ffffff"><input name="surplus" type="text" class="inputBg" id="ECS_SURPLUS" size="10" value="<?php echo empty($this->_var['order']['surplus']) ? '0' : $this->_var['order']['surplus']; ?>" kjAmount="<?php echo $this->_var['kj_goods_amount']; ?>" kjTax="<?php echo $this->_var['kj_goods_tax']; ?>" onblur="changeSurplus(this)" <?php if ($this->_var['disable_surplus']): ?>disabled="disabled"<?php endif; ?> />
                        <span style="margin-left:12px;">您的余额： ￥<?php echo empty($this->_var['your_surplus']) ? '0' : $this->_var['your_surplus']; ?></span> <span id="ECS_SURPLUS_NOTICE" class="notice" style="color:#EF5A2C;"></span></td>
                </tr>
                <?php endif; ?>
...
?>
        ...
        <div class="flowBox">
            ...
            <?php if ($this->_var['is_binding'] == 0): ?>
            <a href="<?php echo $this->_var['beian_link']; ?>" id="kj_beian" onclick="return confirm('消费者需要在宁波跨境贸易电子商务服务平台——跨境购（简称：跨境购平台）进行实名注册，填写真实身份信息并关联本网站用户名，才能购买本网站跨境购商品。（如已在跨境购平台注册过，只需关联本网站用户名即可。）')">
                <img src="themes/ningbo/images/bnt_subOrder.gif" />
            </a>
            <?php else: ?>
            <input type="hidden" name="goods_str" value="<?php echo $this->_var['goods_str']; ?>"/>
            <input type="hidden" name="kj_goods_amount" value="<?php echo $this->_var['kj_goods_amount']; ?>"/>
            <input type="hidden" name="kj_goods_weight" value="<?php echo $this->_var['kj_goods_weight']; ?>"/>
            <input type="hidden" name="kj_goods_number" value="<?php echo $this->_var['kj_goods_number']; ?>"/>
            <input type="hidden" name="kj_goods_tax" value="<?php echo $this->_var['kj_goods_tax']; ?>"/>
            <input type="image" src="themes/ningbo/images/bnt_subOrder.gif" />
                ...
                <td width="50%" valign="top" bgcolor="#ffffff">
                    ...
                    <form action="flow.php?step=login" method="post" name="loginForm" id="loginForm" onsubmit="return checkLoginForm(this)">
                        <table width="90%" border="0" cellpadding="8" cellspacing="0" bgcolor="#B0D8FF" class="table">
                            ...
                            <tr>
                                <td bgcolor="#ffffff" colspan="2" align="center"><a href="user.php?act=qpassword_name" ><?php echo $this->_var['lang']['get_password_by_question']; ?></a>   <a href="user.php?act=get_password"><?php echo $this->_var['lang']['get_password_by_mail']; ?></a></td>
                            </tr>
