...
?>
                <span style="position:absolute;top:120px;left:65px;">
                	...
                	<select name="goods_region_2" onchange="goods_region2(this);"><option value="0">1.选择省份</option><?php echo $this->_var['regions']; ?></select><span id="cnclist"><select name="goods_region_3" onchange="goods_region3(this);"><option value="0">2.选择城市</option></select></span></form>
                </span>
                <span id="region_goods"><ul><?php 
$k = array (
  'name' => 'ads',
  'id' => '21',
  'num' => '3',
...
function show_region_goods(result){
	...
	if (len > 0){
		...
		for (var i = 0; i < len; ++i)
		{
		   res += '<li style="height:100px;"><div class="kjContInner" style="display: block;"><div class="kjAdImg"><a title="' + result[i].goods_name + '" target="_blank" href="' + 'goods.php?id=' + result[i].goods_id + '"><img alt="' + result[i].goods_name + '" src="' + result[i].goods_thumb + '"></a></div><a class="kjAdCover2" title="' + result[i].goods_name + '" target="_blank" href="' + 'goods.php?id=' + result[i].goods_id + '"><img src="themes/ningbo/images/empty.png"></a><ul style="width:165px"><li class="yh18 kjAdCred"><span class="kjAdPrice">￥' + result[i].shop_price + '</span><span style="display:none"></span></li><li class="kjAdCred" style="height:12px;"><span style="display:none"></span></li><li style="width:100%;" class="kjAdName">' + result[i].goods_name + '</li></ul></div></li>';
		}
		res += '</ul>';
	}
	else{
