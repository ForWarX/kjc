...
{
	$email_to = "kefu@americoclub.com";
	$email_subject = "[AMERICOCLUB]B2B Membership Application";
	...
	if (isset($_FILES['yyzz'])){
		...
		upload($_FILES['yyzz'],$company,'yyzz','jpg','b2bmembers');
	}
	if (isset($_FILES['zzjgdmz'])){
		include_once('upload.php');
		upload($_FILES['zzjgdmz'],$company,'zzjgdmz','jpg','b2bmembers');
	}
	if (isset($_FILES['swdjz'])){
		include_once('upload.php');
		upload($_FILES['swdjz'],$company,'swdjz','jpg','b2bmembers');
	}
...
if ($_REQUEST['act'] == 'list')
{
    ...
    if ($count > 0)
    {
		...
		/*foreach ($wholesale_list as $deal){
			$sql = 'SELECT * FROM ';
		}*/
		...
		$smarty->assign('transfer', get_bank_transfer());
...
elseif ($_REQUEST['act'] == 'add_to_cart')
{
    ...
    foreach ($goods_list as $goods_key => $goods)
    {
        ...
        $sql = "SELECT attr_value FROM " . $ecs->table('goods_attr') .
                        " WHERE goods_id = " . $wholesale['goods_id'] . " AND attr_id = 6";
		
		$shipping_raw = $db->getOne($sql);
		if(!is_null($shipping_raw)){
			$shipping = floatval(substr($shipping_raw, 4));
		}
		else{
			$shipping = 0;	
		}
		
		$total_shipping = $goods['number'] * $shipping;
		
		$final_total = $total + $total_shipping;
		...
		$_SESSION['wholesale_goods'][] = array(
			...
			'formated_shipping'		=> price_format($total_shipping, false),
			'formated_final_total'	=> price_format($final_total, false),
