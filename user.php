...
'account_deposit', 'pcoin_deposit', 'account_log', 'account_detail', 'act_account', 'act_pcoin_account', 'pay', 'default', 'bonus', 'group_buy', 'group_buy_detail', 'affiliate', 'comment_list','validate_email','track_packages', 'transform_points','qpassword_name', 'get_passwd_question', 'check_answer','kj_return_exchange','doc_upload');
...
elseif ($action == 'order_detail')
{
    ...
    if ($order === false)
    {
        ...
        if($order['order_type'] == 1){//跨境订单使用
            $order['pay_online']= '<div style="text-align:center"><input type="button" onclick="window.open(\'flow.php?step=pay_order&order_id='.$order['order_id'].'&logId='.$order['log_id'].'\')"  value="立即使用支付宝支付" /></div>';
        }
    ...
    foreach ($goods_list AS $key => $value)
    {
     ...
     /* 设置能否修改使用余额数 */
    if ($order['order_amount'] > 0 &&  $order['order_type'] != 1)//跨境订单不能使用余额支付
    {
...
elseif ($action == 'cancel_order')
{
    ...
    $order_id = isset($_GET['order_id']) ? intval($_GET['order_id']) : 0;
                ...
                /*接口改造 增加 关闭海关订单
                $sql = 'SELECT order_type,order_sn,pay_status  FROM ' . $ecs->table('order_info') . ' WHERE order_id = ' . $order_id;
		$order = $db->getRow($sql);
                if($order['order_type']==1 && $order['pay_status']==2){
                    ...
                    $mftRet=  queryOrderNoList($merchantCode, $merchantName, $order['order_sn'], $orderFrom);
                    if($mftRet['code']=='200'){
                        $mftNo=$mftRet["result"];
                        $closeRt=closeOrder($merchantCode,$merchantName,$mftNo);
                        if($closeRt['code']!='200'){//订单取消失败
                            show_message('订单取消失败，'.$closeRt['desc'].'请稍后再试。', '', 'user.php?act=order_list', 'warning');
                            exit;
                        }
                    }else{//201查询不到申报单号时的操作
                        if($mftRet==null){
                            $rtMsg='订单取消失败，服务器连接异常，请稍后再试。';
                        }else{
                            $rtMsg='订单取消失败，'.$mftRet['desc'].'，请稍后再试。';
                        }
                        show_message($rtMsg, '', 'user.php?act=order_list', 'warning');
                        exit;
                    }
                }//end*/
...
elseif ($action == 'act_pcoin_account'){
    ...
    include_once(ROOT_PATH .'includes/lib_payment.php');
      ...
      //记录支付log
      $order['log_id'] = insert_pay_log($surplus['rec_id'], $order['order_amount'], $type=2, 0);//需要主意这的支付类型，默认的有两个0和1，这里把这个设置成2去边是积分充值。
...
elseif ($action == 'act_account')
{
    ...
    else
    {
		...
		if ($payment_info['pay_name'] == "paypal" || $payment_info['pay_name'] == "支付宝"){
      		$order['order_sn']       = "余额充值";
		}
		else{
			$order['order_sn']       = $surplus['rec_id'];
...
elseif ($action == 'merge_order')
{
    ...
    if(strlen($from_order) > 0 && strlen($to_order) > 0){//购物车改造
        $sql = "SELECT order_type FROM " .$GLOBALS['ecs']->table('order_info')." WHERE order_sn = '$from_order'";
        $from_type = $GLOBALS['db']->getOne($sql);
        $sql = "SELECT order_type FROM " .$GLOBALS['ecs']->table('order_info')." WHERE order_sn = '$to_order'";
        $to_type = $GLOBALS['db']->getOne($sql);
        if($from_type==1 || $to_type==1){
             $GLOBALS['err']->add('合并订单中含有跨境购订单，无法合并！');
...
elseif ($action == 'doc_upload')
{
	...
	$smarty->assign('is_identified', is_identified($user_id));
	$smarty->display('user_transaction.dwt');
	
}
elseif ($action == 'submit_id')
{
	if (!empty($_POST['name'])){
		$name = $_POST['name'];
		if (isset($_FILES['sfz'])){
			include_once('upload.php');
			upload($_FILES['sfz'],$name,'','jpg','ids');
			$GLOBALS['db']->query("UPDATE " .$GLOBALS['ecs']->table('users'). " SET ".
           "is_identified  = 1 WHERE user_id = " . $user_id);
			show_message('上传成功', '回到首页', 'index.php');
		}	
	}
}
elseif ($action == 'submit_license')
{
	if (!empty($_POST['company'])){
		$company = $_POST['company'];
		$counter = 0;
		if (isset($_FILES['yyzz'])){
			include_once('upload.php');
			upload($_FILES['yyzz'],$company,'yyzz','jpg','licenses');
			$counter = 1;
		}
		if (isset($_FILES['zzjgdmz'])){
			include_once('upload.php');
			upload($_FILES['zzjgdmz'],$company,'zzjgdmz','jpg','licenses');
			$counter = 2;
		}
		if (isset($_FILES['swdjz'])){
			include_once('upload.php');
			upload($_FILES['swdjz'],$company,'swdjz','jpg','licenses');
			$counter = 3;
		}
		if ($counter == 3){
			show_message('上传成功', '回到首页', 'index.php');
			...
			show_message('上传失败', '回到首页', 'index.php');
		}	
	}
}
elseif ($action == 'kj_return_exchange')
{
	if (empty($_GET['is_ajax']))
	{
		...
		foreach ($orders as $one_order){
			if ($one_order['order_status'] == '已确认,已付款,已发货' || $one_order['order_status'] == '已确认,已付款,收货确认'){
				$fomatted_order_list .= '<option value="' . $one_order['order_id'] . '">' . $one_order['order_sn'] . '</option>';
			}
		}
		$smarty->assign('orders', $fomatted_order_list);
		$smarty->display('user_transaction.dwt');
	}
	elseif (!empty($_GET['get_goods']))
	{
		...
		if (!empty($order_id))
		{
			$goods_where .= " AND `order_id` = '$order_id'";
		}
	
		$sql = 'SELECT `goods_id`, `goods_name` , `goods_number` FROM ' . $ecs->table('order_goods') . ' WHERE 1 ' . $goods_where . ' LIMIT 50';
		
		die($json->encode($db->getAll($sql)));
	}
	elseif (!empty($_GET['get_quantity']))
	{
		include_once(ROOT_PATH . 'includes/cls_json.php');
		$json = new JSON();
		$order_id = intval($_GET['order_id']);
		...
		if (!empty($good_id))
		{
			$goods_where .= " AND `goods_id` = '$good_id'";
		}
	
		$sql = 'SELECT `goods_number` FROM ' . $ecs->table('order_goods') . ' WHERE 1 ' . $goods_where . ' LIMIT 50';
		
		die($json->encode($db->getOne($sql)));
	}
	elseif (!empty($_GET['start'])){
		...
		if(strpos($info_str,';') !== false){
			...
			//{order_id, good_id, quantity, flag}
	...
	elseif (!empty($_GET['get_rejected'])){//退换货查询
		...
		$result = GetRejected($order_info['order_sn'],$order_info['invoice_no'],$flag);
		$xml=$result;*/
                $merchantCode = $GLOBALS['_LANG']['merchantCode'];
                $merchantName = $GLOBALS['_LANG']['merchantName'];
                $waybillNo = $_GET['waybillNo'];
                $orderFrom = "0000";
                include_once(ROOT_PATH.'report/orderReportNew.php');
                $result=  queryOrderNoList($merchantCode, $merchantName, $order['order_sn'], $orderFrom); 
                if($result['code']=='200'){
                    $mftNo=$result["result"];
                    $result=queryReturnsOrderStatus($mftNo, $waybillNo);
                }
                if($result==null){
                    $reMg='服务器连接异常，请稍后再试。';
                }else{
...
function kj_reject($order_no,$waybill_no,$flag,$rejected_goods){
	//退换货申请
	...
	$ch = curl_init();
	curl_setopt($ch, CURLOPT_URL,"http://i.trainer.kjb2c.com/msg/rejectedmsg.do");
	curl_setopt($ch, CURLOPT_POST, 1);
	
	$OrgOrder_xmlstr="<?xml version='1.0' encoding='UTF-8'?><Message><Header><CustomsCode>" . $customs_code . "</CustomsCode><CreateTime>" . $current_time . "</CreateTime></Header><Body><RejectedInfo><OrderNo>".$order_no."</OrderNo><WaybillNo>".$waybill_no."</WaybillNo><Flag>".$flag."</Flag>".$rejected_goods."</RejectedInfo></Body></Message>";
