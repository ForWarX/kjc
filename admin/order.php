...
elseif ($_REQUEST['act'] == 'delivery_ship')
{
    ...
    $order = order_info($order_id);
		...
		$delivery_order['invoice_no'] = $delivery['invoice_no'];
    ...
    elseif ('cancel' == $operation)
    {
		...
		$order_type = $order['order_type'];
                ...
                if($order_type==1 && $pay_status==2){
                    ...
                    if($mftRet['code']=='200'){
                        $mftNo=$mftRet["result"];
                        $clost_result=closeOrder($merchantCode,$merchantName,$mftNo);//订单关闭结果：{"code":"201","desc":"该订单仓库已打包，不能关闭，请联系仓库。","result":null} 
                        if($clost_result['code']!='200'){
                            /* 操作失败 */
                            continue;
                        }
                    }else{
...
elseif ($_REQUEST['act'] == 'operate_post')
{
    ...
    elseif ('split' == $operation)
    {
		...
		$sql = "SELECT concat(IFNULL(c.region_name, ''), '  ', IFNULL(p.region_name, ''), " .
                ...
                "FROM " . $ecs->table('order_info') . " AS o " .
                    ...
                    "LEFT JOIN " . $ecs->table('region') . " AS d ON o.district = d.region_id " .
                "WHERE o.order_id = '$order[order_id]'";
                $region_str = $db->getOne($sql);
		$delivery_order['region'] = array_slice(explode('  ', $region_str),1);
		
		$zxs = array('北京', '天津', '重庆', '上海');
		$mz = array('内蒙古', '西藏');
		if (!in_array($delivery_order['region'][0], $zxs)){
			if(in_array($delivery_order['region'][0], $mz)){
				$delivery_order['region'][0] .= '自治区';
			}
			elseif($delivery_order['region'][0] == '广西'){
				$delivery_order['region'][0] .= '壮族自治区';
			}
			elseif($delivery_order['region'][0] == '宁夏'){
				$delivery_order['region'][0] .= '回族自治区';
			}
			elseif($delivery_order['region'][0] == '新疆'){
				$delivery_order['region'][0] .= '维吾尔自治区';
			}
			else{
				$delivery_order['region'][0] .= '省';
			}
		}
		
		$_delivery['region'] = array($delivery_order['region'][0], $delivery_order['region'][1], $delivery_order['region'][2]);
		//kj_logistics($_delivery);//接口改造 删除提交运单
		
		//$delivery_id = $db->insert_id();//接口改造 代码放到3769行
        if ($delivery_id)
        {
    ...
    {
                /*  接口改造  */
                $sql = 'SELECT `order_type`,pay_status  FROM ' . $ecs->table('order_info') . ' WHERE order_id = ' . $order_id;
		$order_db = $db->getRow($sql);
                if($order_db['order_type']==1 && $order_db['pay_status']==2){//对已付款的订单取消时需要关闭海关订单
                    $merchantCode = $GLOBALS['_LANG']['merchantCode'];
                    $merchantName = $GLOBALS['_LANG']['merchantName'];
                    $orderFrom = "0000";
                    include_once(ROOT_PATH.'report/orderReportNew.php');
                    ...
                    if($mftRet['code']=='200'){
                        $mftNo=$mftRet["result"];
                        $clost_result=closeOrder($merchantCode,$merchantName,$mftNo);
                        //订单关闭结果：{"code":"201","desc":"该订单仓库已打包，不能关闭，请联系仓库。","result":null} 
                        if($clost_result['code']!='200'){
                            /* 操作失败 */
                            $links[] = array('text' => $_LANG['order_info'], 'href' => 'order.php?act=info&order_id=' . $order_id);
                            if($clost_result==null){
                                $retMsg='服务器连接异常，稍后再试';
                            }else{
                                $retMsg=$clost_result['desc'];
                            }
                            
                            sys_msg($retMsg, 1, $links);
                        }
                    }else{
                        /* 操作失败 无申报单记录*/
                        $links[] = array('text' => $_LANG['order_info'], 'href' => 'order.php?act=info&order_id=' . $order_id);
