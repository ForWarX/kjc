...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit' || $_REQUEST['act'] == 'copy')
{
	...
	/*显示原产地*/
		...
		foreach ($regions as $country){
			$fomatted_region_list .= '<option value="' . $country['region_id'] . '">' . $country['region_name'] . '</option>';
		}
		$smarty->assign('regions', $fomatted_region_list);
		if ($goods['region_id'] != 0){
			$region_info = get_region_info($goods['region_id']);
			if ($region_info['parent_id'] == 0){
				$smarty->assign('country', $region_info);
			}
			else if ($region_info['region_type'] == 2){
				$smarty->assign('city', $region_info);
				$parent_region = get_region_info($region_info['parent_id']);
				$smarty->assign('province', $parent_region);
				$grandparent_region = get_region_info($parent_region['parent_id']);
				$smarty->assign('country', $grandparent_region);
			}
			else if ($region_info['region_type'] == 1){
				$smarty->assign('province', $region_info);
				$parent_region = get_region_info($region_info['parent_id']);
				$smarty->assign('country', $parent_region);
			}
		}
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    /*跨境购备案商品货号*/
	$kj_sn = !empty($_POST['kj_sn']) ? $_POST['kj_sn'] : 5566;
	
	/*原产地*/
	if($_POST['goods_region_3'] == 0){
		if($_POST['goods_region_2'] == 0){
			$region_id = $_POST['goods_region_1'] == 0 ? 0 : intval($_POST['goods_region_1']);
		}
		else{
			$region_id = intval($_POST['goods_region_2']);
		}
	}
	else{
		$region_id = intval($_POST['goods_region_3']);
	}
    ...
    if ($is_insert)
    {
        if ($code == '')
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    ...
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, rank_integral, suppliers_id, kj_sn, region_id)" .
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    "'$brand_id', '$shop_price', '$market_price', '$is_promote','$promote_price', ".
                    "'$promote_start_date', '$promote_end_date', '$goods_img', '$goods_thumb', '$original_img', ".
                    "'$_POST[keywords]', '$_POST[goods_brief]', '$_POST[seller_note]', '$goods_weight', '$goods_number',".
                    " '$warn_number', '$_POST[integral]', '$give_integral', '$is_best', '$is_new', '$is_hot', '$is_on_sale', '$is_alone_sale', $is_shipping, ".
                    " '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$rank_integral', '$suppliers_id', '$kj_sn', '$region_id')";
        }
        else
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    ...
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, extension_code, rank_integral, kj_sn, region_id)" .
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    "'$brand_id', '$shop_price', '$market_price', '$is_promote','$promote_price', ".
                    "'$promote_start_date', '$promote_end_date', '$goods_img', '$goods_thumb', '$original_img', ".
                    "'$_POST[keywords]', '$_POST[goods_brief]', '$_POST[seller_note]', '$goods_weight', '$goods_number',".
                    " '$warn_number', '$_POST[integral]', '$give_integral', '$is_best', '$is_new', '$is_hot', 0, '$is_on_sale', '$is_alone_sale', $is_shipping, ".
                    " '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$code', '$rank_integral', '$kj_sn', '$region_id')";
        }
    }
    else
    {
    ...
    if ($goods_img === false) $goods_img = $original_img;
    ...
    if (isset($img))
    {
        ...
        if (empty($is_url_goods_img))
        {
            ...
            if ($gallery_img === false) $gallery_img = $img;
        }
        else
        {
            $img = $url_goods_img;
        ...
        if ($gallery_thumb === false) $gallery_thumb = $goods_thumb;
...
//-- 修改跨境购货号
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'edit_kj_sn')
{
    check_authz_json('goods_manage');

    $goods_id = intval($_POST['id']);
    $kj_sn = json_str_iconv(trim($_POST['val']));

    /* 检查是否重复 */
    if (!$exc->is_only('kj_sn', $kj_sn, $goods_id))
    {
        make_json_error($_LANG['goods_sn_exists']);
    }
    if ($exc->edit("kj_sn = '$kj_sn', last_update=" .gmtime(), $goods_id))
    {
        clear_cache_files();
        make_json_result(stripslashes($kj_sn));
    }
}
...
elseif ($_REQUEST['act'] == 'edit_goods_weight')
{
    ...
    if($goods_weight < 0 || $goods_num == 0 && $_POST['val'] != "$goods_weight")
    {
        make_json_error('输入有误，请重新输入。');
    }
...
//-- 显示原产地
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'update_region'){
	...
	if (empty($_GET['is_ajax'])){
		...
		if(!empty($_GET['region'])){
			$sub_region = get_regions($_GET['type'],$_GET['region']);
		}
