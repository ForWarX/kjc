...
function goods_list($is_delete, $real_goods=1, $conditions = '')
{
    ...
    if ($result === false)
    {
		...
		$sql = "SELECT g.goods_id, g.cat_id, c.cat_name, g.goods_name, g.goods_type, g.goods_sn, g.kj_sn, g.shop_price, g.goods_weight, g.is_on_sale, g.is_best, g.is_new, g.is_hot, g.sort_order, g.goods_number, g.integral, " .
                    " (promote_price > 0 AND promote_start_date <= '$today' AND promote_end_date >= '$today') AS is_promote ".
                    " FROM " . $GLOBALS['ecs']->table('goods') . " AS g, " . $GLOBALS['ecs']->table('category'). " AS c WHERE g.cat_id = c.cat_id AND g.is_delete='$is_delete' $where" .
