...
class ems
{
    ...
    function query($invoice_sn)
    {
        $str = '<form style="margin:0px" method="post" '.
            'action="http://www.ems.com.cn/ems/order/singleQuery_t" name="queryForm_' .$invoice_sn. '" target="_blank">'.
            '<input type="hidden" id="mailnum_input" name="mailNum" value="' .$invoice_sn. '" />'.
            '<a href="javascript:document.forms[\'queryForm_' .$invoice_sn. '\'].submit();">' .$invoice_sn. '</a>'.
            '<input type="hidden" id="vernum_input" name="checkCode" value="0568792906411" />'.
            '</form>';
