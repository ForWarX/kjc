...
function insert_cart_info1()
{
    ...
    $output = $GLOBALS['smarty']->fetch('library/side_cart.lbi');
    return $output;
}
/**
 * 调用指定的广告位的广告
    ...
    foreach ($res AS $row)
    {
        ...
        switch ($row['media_type'])
        {
            case 0: // 图片广告
                ...
                $ads[] = array('value' => "<a href='affiche.php?ad_id=$row[ad_id]&amp;uri=" .urlencode($row["ad_link"]). "'
                target='_blank'><img src='$src' width='" .$row['ad_width']. "' height='$row[ad_height]'
                border='0' /></a>", 'name' => $row['ad_name']);
                break;
            case 1: // Flash
                $src = (strpos($row['ad_code'], 'http://') === false && strpos($row['ad_code'], 'https://') === false) ?
                        DATA_DIR . "/afficheimg/$row[ad_code]" : $row['ad_code'];
