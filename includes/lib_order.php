 ...
 * @return  array
 */
function order_fee($order, $goods, $consignee, $kj_goods_amount=0.0, $kj_goods_tax=0.0, $kj_goods_weight=0.0, $kj_goods_number=0)
{
    /* 初始化订单的扩展code */
	...
	/* 税费计算的修改 2016.01.19，有订单金额判断修改为使用订单税费进行判断
	 if ($kj_goods_amount > 500){
			$total['tax'] = $kj_goods_tax;
			
			$fp = fopen("ordersubmit.txt","a");
			flock($fp, LOCK_EX) ;
			fwrite($fp,"改了税额：".$total['tax']."\n");
			flock($fp, LOCK_UN);
			fclose($fp);
	} */
    /* 2016.4.8税改取消50元免征额度
    ...
    */
    ...
    if ($order['shipping_id'] > 0 && $total['real_goods_count'] > 0)
    {
        ...
        if (!empty($shipping_info))
        {
			...
			$total['kj_shipping_fee'] = $kj_shipping_fee;
 ...
 */
function cart_goods_selected($type = CART_GENERAL_GOODS)
{
    $sql = "SELECT rec_id, user_id, goods_id, goods_name, goods_sn, goods_number, " .
            "market_price, goods_price, goods_attr, is_real, extension_code, parent_id, is_gift, is_shipping, kj_sn, " .
...
function addto_cart($goods_id, $num = 1, $spec = array(), $parent = 0)
{
    ...
    $sql = "SELECT g.goods_name, g.goods_sn, g.is_on_sale, g.is_real, ".
                ...
                "g.goods_number, g.is_alone_sale, g.is_shipping, g.kj_sn,".
 ...
 * @param   int     $type   类型：默认普通商品
 */
function clear_select_cart($type = CART_GENERAL_GOODS)
{
    $sql = "DELETE FROM " . $GLOBALS['ecs']->table('cart') .
            " WHERE session_id = '" . SESS_ID . "' AND rec_type = '$type' AND is_selected=1 ";
    $GLOBALS['db']->query($sql);
}
...
function get_cart_goods()
{
    ...
    if($row['is_selected']==1){//购物车改造
        ...
        {
            //$real_goods_count++;
            if($row['is_selected']==1) $real_goods_count++;//购物车改造
        }
        else
        {
            //$virtual_goods_count++;
             if($row['is_selected']==1) $virtual_goods_count++;//购物车改造
        }
...
function compute_discount()
{
    ...
    $sql = "SELECT c.goods_id, c.goods_price * c.goods_number AS subtotal, g.cat_id, g.brand_id " .
            ...
            "AND c.is_gift = 0 " .
            "AND rec_type = '" . CART_GENERAL_GOODS . "'";
    $sql.=" AND c.is_selected=1 ";//跨境城购物车改造
