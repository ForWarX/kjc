...
if(!empty($_REQUEST['region'])){
	...
	//通过当前ip获取信息
	 $getLocation = getLocation(real_ip());
	 if($getLocation != ''){
	 $currentInfo = json_decode(substr(strstr($getLocation, '{'),0,-1), true);
	 $sql = "SELECT region_id FROM " . $ecs->table('region') . " WHERE region_name = " . "'" . $currentInfo['city'] . "'";
	$ip_region = $db->GetOne($sql);}
	
	if(!empty($ip_region)){
		
		$_SESSION['region'] = $ip_region;
	}
	else{$_SESSION['region'] = 388;}
}
if (!empty($_SESSION['region']))
{
	$_CFG['region'] = $_SESSION['region'];
}
$this_region = get_region_info($_CFG['region']);
if (isset($smarty))
{
	...
	$smarty->assign('provinces', $fomatted_region_list);
	$smarty->assign('this_region', $this_region['region_name']);
	//$smarty->assign('this_region', $currentInfo['city']);
}
