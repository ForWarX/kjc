...
function kj_logistics($delivery){
	//提交运单
	...
	//收货地址
	if(strpos($delivery['address'],$delivery['region'][2]) !== false){
		if(strpos($delivery['address'],$delivery['region'][1]) !== false){
			if(strpos($delivery['address'],$delivery['region'][0]) !== false){
				$consignee_addr = $delivery['address'];
			}
			else{
				$consignee_addr = $delivery['region'][0] . $delivery['address'];
			}
		}
		else{
			$consignee_addr = $delivery['region'][0] . $delivery['region'][1] . $delivery['address'];
		}	
	}
	else{
		$consignee_addr = $delivery['region'][0] . $delivery['region'][1] . $delivery['region'][2] . $delivery['address'];
	}
	
	//提交进口运单
	$ch = curl_init();
	curl_setopt($ch, CURLOPT_URL,"http://i.trainer.kjb2c.com/msg/logisticsmsg.do");
	curl_setopt($ch, CURLOPT_POST, 1);
	
	$logistics_xmlstr="<?xml version='1.0' encoding='UTF-8'?><Message><Header><CustomsCode>" . $customs_code . "</CustomsCode><OrgName>" . $org_name . "</OrgName><CreateTime>" . $current_time . "</CreateTime></Header><Body><Logistics><LogisticsNo></LogisticsNo><OrderNo>" . $delivery['order_sn'] . "</OrderNo><LogisticsName>". $delivery['shipping_name'] ."</LogisticsName><Consignee>" . $delivery['consignee'] . "</Consignee><Province>" . $delivery['region'][0] . "</Province><City>" . $delivery['region'][1] . "</City><District>" . $delivery['region'][2] . "</District><ConsigneeAddr>" . $consignee_addr . "</ConsigneeAddr><ConsigneeTel>" . $delivery['tel'] . "</ConsigneeTel><GoodsName></GoodsName></Logistics></Body></Message>";
	
	$fp = fopen("logistics.txt","a");
	flock($fp, LOCK_EX) ;
	fwrite($fp,"字串：".$logistics_xmlstr."\n");
	flock($fp, LOCK_UN);
	fclose($fp);
	
	$post_fields = array("userid" => $userid, "timestamp" => $current_time, "sign" => $sign, "xmlstr" => $logistics_xmlstr);
