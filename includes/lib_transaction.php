...
function get_user_orders($user_id, $num = 10, $start = 0)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $arr[] = array('order_id'       => $row['order_id'],
                       ...
                       'handler'        => $row['handler'],
                       'pay_status'     => $row['pay_status'],//支付改造
                       'orderStatus'    =>$orderStatus  //支付改造
                );
 ...
 * @return  array       $order_list     订单列表
 */
function get_user_kj_orders($user_id, $num = 10, $start = 0)
{
    /* 取得订单列表 */
    $arr    = array();
    ...
    $sql = "SELECT order_id, order_sn, order_status, shipping_status, pay_status, add_time, " .
           "(goods_amount + shipping_fee + insure_fee + pay_fee + pack_fee + card_fee + tax - discount) AS total_fee ".
           " FROM " .$GLOBALS['ecs']->table('order_info') .
           " WHERE user_id = '$user_id' AND order_type=1 ORDER BY add_time DESC";
    $res = $GLOBALS['db']->SelectLimit($sql, $num, $start);
...
function is_identified($user_id){
	return $GLOBALS['db']->getOne("SELECT is_identified FROM " .$GLOBALS['ecs']->table('users') . " WHERE user_id = '" . $user_id . "'");
}
?>