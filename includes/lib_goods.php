...
function assign_cat_goods($cat_id, $num = 0, $from = 'web', $order_rule = '')
{
    ...
    foreach ($res AS $idx => $row)
    {
        ...
        $goods[$idx]['url']          = build_uri('goods', array('gid' => $row['goods_id']), $row['goods_name']);
		 ...
		 $goods[$idx]['cat_name']    = $GLOBALS['db']->getOne('SELECT cat_name FROM ' . $GLOBALS['ecs']->table('category'). ' WHERE cat_id = '. $row['cat_id']);
    }
...
function is_kj_product($cat_id){
	 ...
	 else if($cat_name == '立即购'){
		 return 2;
	 }
	 else if($cat_name == '中国购'){
	 	 return 3;
	 }
	 else{
		 return FALSE;	
 ...
 * @return      array
 */
function get_region_goods($region,$order='',$limit=99){
	$sql = "SELECT * FROM " .$GLOBALS['ecs']->table('goods'). " WHERE region_id = '$region' " . $order . " LIMIT " . $limit;
